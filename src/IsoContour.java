...
public class IsoContour {
	...
	ArrayList<PVector> vertexPositions;
	
	
	static final int edgeToVertexA[] = { 0, 1, 2, 3, 0, 1, 2, 3 };
	static final int edgeToVertexB[] = { 0, 1, 2, 3, 1, 2, 3, 0 };
	...
	public IsoContour( PApplet _theParent, PVector _start, PVector _end, int _cols, int _rows ) {
		...
		vertexPositions = new ArrayList<PVector>();
		numVertices = 0;
	...
	public void construct() {
		...
		for ( int i = 0; i <= cols; i++ ) {
			for ( int j = 0; j <= rows; j++ ) {
				vertexes[count] = new PVector(start.x + colDistance*i, start.y + rowDistance*j);
				count++;
			}
		}
		this.clear();
	...
	public void addPoint( PVector _pt, float weight ) {
		...
		for ( int i = 0; i < vertexes.length; i++ ) {
			float d = PApplet.max(1.f, _pt.dist(vertexes[i]));
			vertexValues[i] += (1.f / PApplet.pow(d, 2)) * weight;
		}
	}
	...
	public void plotGrid() {
		...
		float rowDistance = (end.y - start.y) / rows;
		for ( int i = 0; i <= cols; i++ ) {
			theParent.line( start.x + colDistance*i, start.y, start.x + colDistance*i, end.y );
		}
		for ( int i = 0; i <= rows; i++ ) {
			theParent.line( start.x, start.y + rowDistance*i, end.x, start.y + rowDistance*i );
		}  
	}
				...
				for ( int v = 0; v < 3; v++ ) {
					...
					if ( spread != 0 ) {
						if ( vertexValues[ref[num1]] < vertexValues[ref[num2]] ) {
							vtx1P = 1.f - PApplet.abs((threshold - vertexValues[ref[num1]]) / spread);
							vtx2P = 1.f - PApplet.abs((vertexValues[ref[num2]] - threshold) / spread);
						}
						else {
							vtx1P = 1.f - PApplet.abs((vertexValues[ref[num1]] - threshold) / spread);
							vtx2P = 1.f - PApplet.abs((threshold - vertexValues[ref[num2]]) / spread);
						}
					}
					else {
						vtx1P = .5f;
					...
					vertexPositions.add( new PVector(x, y) );
					numVertices++;
				}
			}
		}
